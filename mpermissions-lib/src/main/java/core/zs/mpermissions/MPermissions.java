...
public class MPermissions {
    ...
    public static void onRequestPermissionsResult(Activity activity, int requestCode, String[]
            permissions, int[] grantResults) {
        requestResult(activity, requestCode, permissions, grantResults);
    }

    public static void onRequestPermissionsResult(Fragment fragment, int requestCode, String[]
            permissions, int[] grantResults) {
    ...
    private static void _requestPermissions(Object object, int requestCode, String... permissions) {
        if (!MUtils.isOverMarshmallow()) {
            handleApplySuccess(object, requestCode);
            return;
        }
        List<String> deniedPermissions =
                MUtils.findDeniedPermissions(MUtils.getActivity(object), permissions);
    ...
    private static void handleApplySuccess(Object activity, int requestCode) {
        findPermissionProxy(activity).grant(activity, requestCode);
    }

    private static void handleApplyFalil(Object activity, int requestCode) {
        findPermissionProxy(activity).denied(activity, requestCode);
    }

    private static void requestResult(Object obj, int requestCode, String[] permissions, int[]
            grantResults) {
        ...
        if (deniedPermissions.size() > 0) {
            handleApplyFalil(obj, requestCode);
        }
        else {
            handleApplySuccess(obj, requestCode);
        }
    }
}
